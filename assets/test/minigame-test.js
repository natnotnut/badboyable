document.addEventListener('DOMContentLoaded', function () {
    const ACCESS_KEY = 'oneTimeAccessKey';
    function hasAccessed() {
        return localStorage.getItem(ACCESS_KEY) !== null;
    }
    function markAsAccessed() {
        localStorage.setItem(ACCESS_KEY, 'true');
    }
    if (hasAccessed()) {
        document.getElementById('mg-body').innerHTML = "<div id='limited-access'>Kamu telah mengakses kuis ini sebelumnya, tidak dapat mengakses lagi!</div>";
    } else {
        markAsAccessed();
    }
  });
  ...
  const questions = [
     {
   number:1,        
   question:'Tahun berapakah Nat mulai bermain Maifam?', 
   answer:'2021', 
   options:[
           '2020', 
           '2021', 
     ...
     {
             number:2,
             question: 'Nickname Maifam yang pernah Nat pakai, kecuali...',
             answer: 'Me',
             options: [
           'Bumi',
           'CumiCumi',
     ...
     {
             number:3,
             question: 'Tema apa yang tidak pernah dipakai di grup diskusi Nat?',
             answer: 'Tidak ada dalam pilihan',
             options: [
           'All Of Us Are Bad',
           'Nat Has Come',
     ...
     {
             number:4,
             question: 'Hasil dari 1+1-1&#215;0 adalah...',
             answer: '2',
             options: [
           '0',
           '1',
     ...
     {
             number:5,
             question:'(English) Verb 3 dari kata menyanyi?',
             answer:'sung',
             options:[
           'sang',
           'sing',
  ...
  let totalQuesCount =  `<span><b>Kuis HTS [`+index+`<span>/</span><span>`+questions.length+`</span>]</b></span>`;
  ...
  let scoreTag = `<span>Skor : `+score+` dari `+questions.length+` 🔥</span>`;  
